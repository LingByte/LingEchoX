...
func main() {
	...
	// 系统监控钩子（pkg/utils/system）：
	//
	//   1) StartSystemMonitor   —— 后台 5s 采样一次 CPU/Mem/Disk，写入
	//      atomic.Value，供 /system/status 与 listeners 读取。它自身已
	//      `go func()` 但没有 panic recover，所以包一层 SafeGo 兜底，
	//      防止 gopsutil 在异常平台上 panic 撕掉整个监控通道。
	//
	//   2) Monitor              —— CPU 持续高位时自动 dump pprof 到 ./pprof/。
	//      仅在 PROFILE_AUTO_PPROF=true 时启用，避免开发机被刷文件。
	//
	//   3) StartPyroScope       —— 仅在 PYROSCOPE_URL 已配置时挂；未配置则 no-op。
	//      返回 error 我们仅 warn，不让缺失的可观测能力把进程拉黑。
	logger.SafeGo("system-monitor", system.StartSystemMonitor)
	if utils.GetEnv("PROFILE_AUTO_PPROF") == "true" {
	...
	//   - WriteTimeout: 0 (disabled) — long-poll-ish endpoints (webseat
	//     WS upgrades hijack the conn so this doesn't apply, but JSON
	//     handlers also occasionally take >30s when the LLM/ASR providers
	//     are slow). gin.Recovery + handler-level ctx timeouts manage
	//     misbehaviour; relying on WriteTimeout alone risks killing
	//     slow-but-correct streaming responses mid-flight.
