...
interface PermissionEditorProps {
  ...
  onChange: (permissions: { [key: string]: string[] }) => void
}

const PermissionEditor = ({ permissions, onChange }: PermissionEditorProps): JSX.Element => {
  const togglePermission = (resource: string, action: string) => {
    const newPermissions = { ...permissions }
    if (!newPermissions[resource]) {
      newPermissions[resource] = []
    }
    const actions = [...newPermissions[resource]]
  ...
  return (
    <div className="space-y-4">
      {PERMISSION_RESOURCES.map((resource) => (
        <div key={resource.value} className="border border-slate-200 dark:border-slate-700 rounded-lg p-3">
          <div className="font-medium text-sm text-slate-700 dark:text-slate-300 mb-2">
            {resource.label}
          </div>
          <div className="flex flex-wrap gap-2">
            {PERMISSION_ACTIONS.map((action) => {
              const isChecked = permissions[resource.value]?.includes(action.value) || false
              return (
                <label
                  key={action.value}
