...
/** 通话 AI 分析（ASR + LLM），与后端 sip-center/call-analysis 一致 */
export interface CallAnalysisMeta {
  source: string
  filename?: string
  audio_url?: string
  pcm_duration_sec: number
  /** 送入 ASR 的 PCM 采样率，与 ASR 模型一致（如 8k_zh→8000，16k_zh→16000） */
...
export interface CallAnalysisASR {
  provider: string
  transcript: string
}

export interface CallAnalysisExportDoc {
  version: string
  id: string
  created_at: string
  meta: CallAnalysisMeta
  asr: CallAnalysisASR
  llm_analysis?: unknown
  llm_raw?: string
}

export interface CallAnalysisCreateData {
  id: string
  meta: CallAnalysisMeta
  asr: CallAnalysisASR
  llm: unknown
  llm_raw: string
  export_doc: CallAnalysisExportDoc
}

/** 须 ≥ 服务端 ASR（常接近实时时长）+ LLM；与 cmd/server HTTP 长超时对齐 */
const callAnalysisTimeoutMs = 45 * 60 * 1000

/** multipart：字段 audio（文件）或 audio_url（表单） */
export function runCallAnalysisMultipart(form: FormData) {
  return post<CallAnalysisCreateData>('/sip-center/call-analysis', form, { timeout: callAnalysisTimeoutMs })
}

/** JSON：{ "audio_url": "https://..." } */
export function runCallAnalysisByURL(audioUrl: string) {
  return post<CallAnalysisCreateData>('/sip-center/call-analysis', { audio_url: audioUrl }, { timeout: callAnalysisTimeoutMs })
}

export function getCallAnalysisResult(id: string) {
  return get<CallAnalysisExportDoc>(`/sip-center/call-analysis/${id}`)
}
