...
//
// Field layout is the same as the legacy pkg/sip/conversation.VoiceEnv
...
type VoiceEnv struct {
	...
	// TransferConfirmCount: user must express transfer intent this many
	// times (separate final transcripts) before dial. 0 = use default.
...
func VoiceEnvFromJSON(asrRaw, ttsRaw, llmRaw, realtimeRaw []byte, voiceMode string) (VoiceEnv, error) {
	...
	out.TransferConfirmCount = parseTransferConfirmCount(rm, lm)
...
// parseTransferConfirmCount returns the first positive
// transferConfirmCount / transfer_confirm_count found across the
// provided maps (typically realtime then llm tab). Zero when neither
// map has a positive value — callers apply their own default.
