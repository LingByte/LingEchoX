...
func TestVoiceEnvFromJSON_CozeFlattening(t *testing.T) {
	env, err := VoiceEnvFromJSON(
		nil, nil,
		mustJSON(t, map[string]any{"provider": "coze", "apiKey": "k", "botId": "b1", "userId": "u1", "baseUrl": "https://api.coze.cn"}),
		nil,
		"pipeline",
	)
	if err != nil {
	...
	// Coze flattening: LLMBaseURL becomes a JSON blob carrying botId/userId/baseUrl.
	if !strings.Contains(env.LLMBaseURL, `"botId":"b1"`) {
		t.Errorf("coze flatten: %q", env.LLMBaseURL)
	}
}
...
func TestLLMReady_PerProvider(t *testing.T) {
	cases := []struct {
		...
		{"alibaba: key+appid", VoiceEnv{LLMProvider: "alibaba", LLMAPIKey: "k", LLMAppID: "a"}, true},
		{"alibaba: missing appid", VoiceEnv{LLMProvider: "alibaba", LLMAPIKey: "k"}, false},
		{"coze: key+baseurl", VoiceEnv{LLMProvider: "coze", LLMAPIKey: "k", LLMBaseURL: "https://x"}, true},
		{"coze: missing baseurl", VoiceEnv{LLMProvider: "coze", LLMAPIKey: "k"}, false},
...
func TestResolve_TenantIDZero(t *testing.T) {
	resetLoaderForTest(t)
	SetLoader(func(context.Context, uint) ([]byte, []byte, []byte, []byte, string, bool) {
...
func TestResolve_LoaderHappy(t *testing.T) {
	resetLoaderForTest(t)
	SetLoader(func(_ context.Context, tid uint) ([]byte, []byte, []byte, []byte, string, bool) {
		if tid != 7 {
...
func TestResolve_LoaderHappyWithJSONError(t *testing.T) {
	resetLoaderForTest(t)
	SetLoader(func(context.Context, uint) ([]byte, []byte, []byte, []byte, string, bool) {
		return []byte("{not json"), nil, nil, nil, "pipeline", true
	})
...
func TestParseTransferConfirmCount_FirstPositiveWins(t *testing.T) {
	if got := parseTransferConfirmCount(); got != 0 {
		t.Errorf("no maps → %d, want 0", got)
	}
	got := parseTransferConfirmCount(
		map[string]any{},
		map[string]any{"transferConfirmCount": float64(3)},
		map[string]any{"transferConfirmCount": float64(5)},
	)
	if got != 3 {
		t.Errorf("got %d, want 3 (first positive wins)", got)
	}
}
