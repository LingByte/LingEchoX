...
//
// Engine never sees SIP messages, transactions, dialogs or RTP. It
// receives PCM through MediaPort and emits PCM the same way. SIP-side
// concerns (codec negotiation, jitter, RTCP, transfer signalling)
// stay in pkg/sip/*; the SIP layer constructs an Engine via Factory
// and calls Attach once per call.
//
...
// any transport (SIP RTP / WebRTC / file / fake) can implement it.
//
// Lifecycle:
//
...
type MediaPort interface {
	...
	// Codec describes the negotiated SIP codec for this call (e.g.
...
// codec. We don't use pkg/sip/sdp types directly here to keep this
// package SIP-free; concrete adapters fill it in.
