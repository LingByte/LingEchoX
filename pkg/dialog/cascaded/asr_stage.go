...
//     Production wiring at the SIP/dialog seam (PR-9d) adapts
		...
		case f, ok := <-inCh:
			...
			if f.Kind == pipeline.KindPCM {
				if err := s.asr.ProcessPCM(ctx, f.PCM.Data); err != nil {
					...
					// Transient ProcessPCM errors are observability
					// only — surface via SetErrorCallback in real
					// implementations. We log at debug to avoid
					// spamming on every transient blip.
