...
// ============
//
// Until PR-9b, barge-in detection lived inside the legacy SIP attach
// helpers (pkg/sip/conversation/voice.go::attachVoiceInner held its
...
// Implemented by *pkg/sip/vad.Detector in production; tests provide
// a deterministic fake. Kept as an interface so cascaded does NOT
// import pkg/sip/vad (which would invert the dialog → sip layering
...
func newVADStage(detector BargeInDetector, isTTSPlaying func() bool, onBargeIn func()) *vadStage {
	return &vadStage{
		detector:     detector,
...
func (s *vadStage) Run(ctx context.Context, in <-chan pipeline.Frame, out chan<- pipeline.Frame, lg engine.Logger) error {
	...
	for {
		...
		case f, ok := <-in:
			...
			if playing && s.detector.CheckBargeIn(f.PCM.Data, true) {
