...
// so the SIP-side adapter can build one corrector / persister and
...
type Engine struct {
	...
	// Optional tail stages — same interfaces as cascaded so the SIP
...
// directly by SIP, not by a dialog/engine factory).
...
func (e *Engine) Attach(ctx context.Context, port engine.MediaPort, lg engine.Logger) (engine.Detach, error) {
	...
	if e.persister != nil {
		stages = append(stages, cascaded.NewPersistStage(e.persister))
