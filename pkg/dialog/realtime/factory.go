...
// Production wiring (in pkg/sip) MUST call SetAgentBuilder during
...
// package-level builder provider (which is set once at SIP bridge
// init).
type factory struct{}

// Build constructs a realtime.Engine. Only engine.ModeRealtime is
// accepted; the legacy bridge owns ModeRealtime today, so production
// wires this factory under a future ModeRealtimeNative once the
// per-call route lands (PR-10b). PR-10a only needs the constructor
// callable from tests.
