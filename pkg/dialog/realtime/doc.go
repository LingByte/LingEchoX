...
// interface (see types.go) that the SIP-side adapter implements via
...
// WebSocket.
//
// PR-10a wires only the engine + agentStage scaffolding. SIP-side
// integration (a native realtime route mirroring
// dialog_engine_native_route.go for cascaded) lands in PR-10b. Stage
// extraction (transfer state machine, welcome gate, AI jitter
// player) and the cross-mode IntentDetectionStage follow in PR-10c+.
package realtime
