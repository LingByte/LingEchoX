...
// SafeGoCtx is like SafeGo but propagates a context for callers that
// want to embed it (the recover wrapper itself doesn't observe ctx —
// it's the caller's responsibility to honour ctx.Done() inside fn).
//
// This overload exists mostly for ergonomic reasons; the safety
// guarantee is identical to SafeGo.
func SafeGoCtx(name string, fn func()) {
	SafeGo(name, fn)
}
