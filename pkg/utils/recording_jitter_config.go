...
// jitter snap 阈值的运行时配置入口。原本在两处硬编码 80ms：
//
//   - pkg/voice/recorder/recorder.go::placePCMTrackBytes（新立体声 recorder）
//   - pkg/utils/sip_recording_wav.go::placeWallPCMTrack（老 SN3→WAV 转码路径）
//
// 抽到这里统一通过 SIP_RECORDING_JITTER_SNAP_MS 配置，让上线时可以临时调整：
//
...
// 集散地（已有 GetEnv 等），其他包都依赖它；recorder 反向依赖 utils 没有循环
// 风险，sip_recording_wav.go 同包直接调用即可。

import "strconv"

// RecordingJitterSnapNs 返回 jitter snap 窗口（纳秒）。
// SIP_RECORDING_JITTER_SNAP_MS 环境变量可覆盖，但被 clamp 到 [10, 500] ms 区间
// 防止误配置导致的极端行为：
//
...
func RecordingJitterSnapNs() int64 {
	...
	raw := GetEnv("SIP_RECORDING_JITTER_SNAP_MS")
	if raw == "" {
