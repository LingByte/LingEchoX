...
func TestWriter_EmitAndReadBack(t *testing.T) {
	...
	rec := NewCallRecord("call-001", "sip", time.Now())
	...
	if got.CallID != "call-001" || got.Transport != "sip" || got.EndStatus != "ok" {
...
func TestWriter_DropsOnFullBuffer(t *testing.T) {
	...
	var dropped int
	for i := 0; i < 5000; i++ {
		r := NewCallRecord("c", "sip", time.Now())
		r.Finalize(time.Now())
		if !w.Emit(r) {
			dropped++
		}
