...
import (
	...
	"github.com/LinByte/VoiceServer/pkg/voice/metrics"
	"go.uber.org/zap"
)
...
type Config struct {
	...
	// with mkdir-p semantics if missing. Default: /var/log/lingechox.
	Dir string
...
func (c *Config) applyDefaults() {
	if c.Dir == "" {
		// Default: <process working dir>/trace. Picked over
		// /var/log/lingechox because LingEchoX is typically run
		// non-root and operators expect logs alongside the
		// repository / artifact bundle, not in a system path that
		// needs special mkdir privileges.
		c.Dir = "trace"
	}
	if c.BaseName == "" {
		c.BaseName = "cdr"
