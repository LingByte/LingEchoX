...
func NewHardwareSession(ctx context.Context, hardwareConfig *HardwareSessionOption) *HardwareSession {
	...
	// 初始化录音器（内存存储）
	if recorder, err := NewAudioRecorder(16000, 1, 16, hardwareConfig.Logger); err != nil {
...
func (s *HardwareSession) saveCallRecording() {
	...
	if s.recorder != nil {
		...
		wavData := s.recorder.GetWAVData()
		if len(wavData) > 0 {
			if storageURL, err := s.uploadRecordingData(wavData); err != nil {
				s.logger.Error("[Session] 上传录音数据失败", zap.Error(err))
			} else {
				s.callRecording.StorageURL = storageURL
			}
		}
...
// uploadRecordingData 上传内存录音数据到存储服务
func (s *HardwareSession) uploadRecordingData(wavData []byte) (string, error) {
	if s.callRecording == nil {
		return "", nil
	}
	...
	s.logger.Info("[Session] 开始上传录音数据",
		zap.String("storageKey", storageKey))

	s.logger.Info("[Session] 录音数据已准备",
