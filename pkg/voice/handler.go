...
// VoiceOptions voice module options.
...
// HardwareOptions compatibility alias for historical callers.
type HardwareOptions = VoiceOptions

// VoiceHandler handles websocket voice sessions.
type VoiceHandler struct {
	logger *zap.Logger
	db     *gorm.DB
}

// NewVoiceHandler create voice handler.
func NewVoiceHandler(db *gorm.DB, logger *zap.Logger) *VoiceHandler {
	if logger == nil {
		logger = zap.L()
	}
	return &VoiceHandler{
...
// NewHardwareHandler compatibility constructor.
func NewHardwareHandler(db *gorm.DB, logger *zap.Logger) *VoiceHandler {
	return NewVoiceHandler(db, logger)
}

// HandlerHardwareWebsocket handler hardware websocket
func (h *VoiceHandler) HandlerHardwareWebsocket(
	ctx context.Context,
