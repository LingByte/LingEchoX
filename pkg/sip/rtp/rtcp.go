...
func (s *Session) startRTCP(rtpLocalPort int) {
	...
	if err != nil {
		...
		if logger.Lg != nil {
			logger.Lg.Warn("rtp/rtcp companion socket bind failed; running without RTCP",
				zap.Int("rtp_port", port),
				zap.Int("attempted_rtcp_port", port+1),
				zap.Error(err))
		}
		return
	}
...
func (s *Session) sendOneRTCP(conn *net.UDPConn) {
	...
	if err != nil {
		if logger.Lg != nil {
			logger.Lg.Warn("rtcp marshal failed", zap.Error(err))
		}
		return
	}
	if _, err := conn.WriteToUDP(raw, remote); err != nil {
		// Don't spam: peers behind asymmetric NAT often refuse RTCP
		// before they've sent anything our way. Log once.
		if !errors.Is(err, net.ErrClosed) && logger.Lg != nil {
			s.rtcpTxWarnOnce.Do(func() {
				logger.Lg.Warn("rtcp send failed (suppressing further)",
