...
// sipRealtimeToolHandler dispatches Qwen-Omni-Realtime function calls for a call leg.
type sipRealtimeToolHandler struct {
	callID          string
	confirmRequired int
	lg              *zap.Logger
...
func newSIPRealtimeToolHandler(callID string, confirmRequired int, lg *zap.Logger, triggerTransfer func(reason string)) realtime.ToolHandler {
	return (&sipRealtimeToolHandler{
		callID:          callID,
...
func (h *sipRealtimeToolHandler) handle(name string, args map[string]any) string {
	switch name {
	case "transfer_to_agent":
		return h.handleTransfer(args)
...
func (h *sipRealtimeToolHandler) handleTransfer(args map[string]any) string {
	allowed, count := sipTransferMayExecute(h.callID, h.confirmRequired)
	if !allowed {
		remaining := h.confirmRequired - count
		if h.lg != nil {
			h.lg.Info("sip voice (realtime): transfer tool blocked (confirm count)",
				zap.String("call_id", h.callID),
				zap.Int("count", count),
				zap.Int("required", h.confirmRequired),
				zap.Any("args", args),
			)
		}
		return toolJSON(transferConfirmToolPayload(count, h.confirmRequired, remaining))
	}
	if h.lg != nil {
		h.lg.Info("sip voice (realtime): transfer tool invoked, deferring dial until playback finishes",
			zap.String("call_id", h.callID),
			zap.Any("args", args),
			zap.Int("confirm_count", count),
		)
	}
	markSIPTransferPending(h.callID)
	return toolJSON(map[string]any{"ok": true, "action": "transfer_requested"})
}
