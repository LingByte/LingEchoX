...
func (leg *outLeg) requestCANCEL() error {
	...
	go func() {
		...
		logger.Info("sip outbound CANCEL: provisional grace expired, forcing send",
			zap.String("call_id", leg.params.CallID),
			zap.Duration("grace", cancelProvisionalGrace),
		)
		_ = leg.fireDeferredCANCELLocked()
	}()
	logger.Info("sip outbound CANCEL queued (waiting for first 1xx)",
		zap.String("call_id", leg.params.CallID),
...
func (leg *outLeg) fireDeferredCANCELLocked() error {
	...
	logger.Info("sip outbound CANCEL sent",
		zap.String("call_id", leg.params.CallID),
		zap.String("request_uri", strings.TrimSpace(leg.params.RequestURI)),
		zap.String("correlation_id", strings.TrimSpace(leg.req.CorrelationID)),
		zap.String("dst_udp", dst.String()),
	)
	...
	go func() {
		...
		for i := 1; i < cancelRetransmitAttempts; i++ {
			...
			if err := leg.sendOnPeer(buildCANCEL(leg.params), d); err != nil {
				logger.Warn("sip outbound CANCEL retransmit failed",
					zap.String("call_id", leg.params.CallID),
					zap.Int("attempt", i+1),
					zap.Error(err))
				return
			}
			logger.Debug("sip outbound CANCEL retransmit",
				zap.String("call_id", leg.params.CallID),
				zap.Int("attempt", i+1),
				zap.Duration("after", interval),
			)
			interval *= 2
