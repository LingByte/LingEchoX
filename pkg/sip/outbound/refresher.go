...
func (leg *outLeg) startRefresherIfUAC(se int, refresher session_timer.Refresher) {
	...
	if se < session_timer.DefaultMinSE {
		...
		logger.Warn("sip outbound refresher rejected sub-90s SE",
	...
	logger.Info("sip outbound session-timer refresher armed",
		zap.String("call_id", leg.params.CallID),
		zap.Int("se", se),
		zap.String("role", "uac"))
	go r.run()
...
func (r *outRefresher) sendUPDATE() bool {
	...
	if err := leg.sendOnPeer(msg, dst); err != nil {
		logger.Warn("sip outbound refresh UPDATE send failed",
			zap.String("call_id", leg.params.CallID),
			zap.Int("cseq", cseq),
			zap.Error(err))
	...
	logger.Debug("sip outbound refresh UPDATE sent",
		zap.String("call_id", leg.params.CallID),
		zap.Int("cseq", cseq),
		zap.Int("se", se))
	return true
...
func (r *outRefresher) handleUPDATEResponse(resp *stack.Message) bool {
	...
	case st >= 200 && st < 300:
		...
		if peerSE, peerRefresher, _ := session_timer.ParseSessionExpires(resp.GetHeader("Session-Expires")); peerSE > 0 {
			...
			if peerRefresher == session_timer.RefresherUAS {
				...
				logger.Info("sip outbound refresher role swapped to uas by peer 200 OK; stopping",
					zap.String("call_id", r.leg.params.CallID))
				return false
			}
		}
	...
	case st == 422:
		...
		if peerMin <= 0 || peerMin > session_timer.HardMaxSE {
			...
			logger.Warn("sip outbound refresh got 422 without usable Min-SE; stopping",
				zap.String("call_id", r.leg.params.CallID),
		...
		if r.retried422 {
			r.mu.Unlock()
			sipMetrics.SessionTimerRefresh(sipMetrics.Refresh422GaveUp)
			logger.Warn("sip outbound refresh 422 again after retry; stopping refresher",
				zap.String("call_id", r.leg.params.CallID))
			return false
		}
		r.retried422 = true
		...
		newSE := r.se
		r.mu.Unlock()
		logger.Info("sip outbound refresh bumped SE after 422",
			zap.String("call_id", r.leg.params.CallID),
			zap.Int("new_se", newSE),
			zap.Int("peer_min_se", peerMin))
	...
	case st == 481:
		...
		logger.Warn("sip outbound refresh 481; stopping refresher",
			zap.String("call_id", r.leg.params.CallID))
		return false
	...
	case st >= 400:
		...
		logger.Warn("sip outbound refresh non-2xx response",
			zap.String("call_id", r.leg.params.CallID),
			zap.Int("status", st),
