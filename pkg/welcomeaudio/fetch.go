...
// audio URL, using a process-local cache. The decodeWAV callback lets
// callers pass their own WAV-bytes-to-PCM decoder (in this codebase:
// conversation.LoadWAVAsPCM16FromBytes) — keeps this package free of
// a dependency cycle on pkg/sip/conversation.
//
// Error semantics:
...
func FetchPCM(ctx context.Context, rawURL string, sampleRate int, decodeWAV func(raw []byte, sampleRate int) ([]byte, error)) ([]byte, error) {
	if decodeWAV == nil {
		return nil, errors.New("decodeWAV callback is required")
	}
	if sampleRate <= 0 {
		sampleRate = 16000
	}
	...
	key := fmt.Sprintf("%s|%d", u.String(), sampleRate)

	// Cache hit (and not expired) — fast path. We re-check expiry
	// inside the loaded entry rather than carrying a separate index.
	...
	resp, err := httpClientForFetch().Do(req)
	if err != nil {
	...
	if len(body) > MaxBytes {
		return nil, fmt.Errorf("%w: body exceeds %d bytes", ErrUnreachable, MaxBytes)
	}
